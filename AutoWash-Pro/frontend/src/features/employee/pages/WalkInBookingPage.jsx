...
function WalkInBookingPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // Lưu ý: việc thu tiền (tiền mặt hoặc mở QR online) chỉ được thực hiện
      // sau khi booking chuyển trạng thái "completed" (đã rửa xe xong), qua
      // màn hình chi tiết booking trong hàng chờ (EmployeeBookingDetailModal
      // -> employeeApi.collectCashPayment / createOnlinePayment). Trước đây
      // chỗ này điều hướng sang trang /employee/payment (dùng chung với
      // trang thanh toán của khách hàng), nhưng trang đó gọi thẳng API
      // POST /payments vốn chỉ cho phép role CUSTOMER nên nhân viên bấm vào
      // sẽ luôn bị lỗi 403. Vì vậy chỉ tạo booking rồi báo cho nhân viên
      // biết bước tiếp theo là thu tiền tại hàng chờ.
        ...
        {successBooking && (
            ...
            >
              ...
              Mã booking:{" "}
                ...
                {successBooking.customerIsGuest === false && (
                    <span>
                Khách quen — đang có{" "}
                      {successBooking.customerTotalPoints ?? 0} điểm tích lũy.
              </span>
                )}

                <span>
              Thu tiền (tiền mặt hoặc QR online) sẽ được thực hiện tại
              trang hàng đợi sau khi xe rửa xong.
            </span>
              </div>
